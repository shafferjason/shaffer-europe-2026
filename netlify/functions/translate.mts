...
export default async (req: Request): Promise<Response> => {
  ...
  if (!apiKey) {
    return json({ error: "Translator is not configured (missing API key)." }, 500);
  }
  ...
  try {
    ...
    return json({ error: "Audio could not be read." }, 400);
  }
  if (audioBytes.length === 0) {
    return json({ error: "Empty recording." }, 400);
  }
  if (audioBytes.length > MAX_AUDIO_BYTES) {
    return json({ error: "Recording too long — keep it under ~30 seconds." }, 413);
  }
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return json({ error: "Could not understand the audio. Try again." }, 502);
    }
    const data = await r.json();
    original = (data.text || "").trim();
  } catch (e) {
    console.error("Transcription error:", e);
    return json({ error: "Translator had trouble hearing that." }, 502);
  }

  if (!original) {
    return json({ error: "Didn't catch any speech — try again." }, 200);
  }
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return json({ error: "Could not translate that. Try again." }, 502);
    }
    const data = await r.json();
    translation = (data.choices?.[0]?.message?.content || "").trim();
  } catch (e) {
    console.error("Translation error:", e);
    return json({ error: "Translator had trouble." }, 502);
  }

  return json({ original, translation, target });
};
